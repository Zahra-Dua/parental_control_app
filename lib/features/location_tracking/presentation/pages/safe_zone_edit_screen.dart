...
import 'package:parental_control_app/features/location_tracking/presentation/blocs/geofence/geofence_event.dart';
import 'package:parental_control_app/features/location_tracking/presentation/blocs/geofence/geofence_state.dart';
...
class _SafeZoneEditScreenState extends State<SafeZoneEditScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (_) => sl<GeofenceBloc>()..add(GeofenceInitForChild(parentId: parentId, childId: widget.childId)),
      child: Scaffold(
        ...
        body: BlocConsumer<GeofenceBloc, GeofenceState>(
          listener: (context, state) async {
            if (state is GeofenceReady) {
              setState(() {
                _center = state.suggestedCenter;
              });
              final controller = await _controller.future;
              controller.animateCamera(CameraUpdate.newLatLngZoom(_center, 14));
            } else if (state is GeofenceSaved) {
