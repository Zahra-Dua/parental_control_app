...
import 'package:parental_control_app/features/location_tracking/presentation/blocs/map/map_event.dart';
import 'package:parental_control_app/features/location_tracking/presentation/blocs/map/map_state.dart';
...
class _MapScreenState extends State<MapScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<MapBloc>()..add(MapStarted(parentId: FirebaseAuth.instance.currentUser!.uid, childId: widget.childId)),
      child: Scaffold(
          ...
          builder: (context, state) {
            ...
            if (state is MapLoaded) {
              if (state.lastLocation != null) {
                target = LatLng(state.lastLocation!.latitude, state.lastLocation!.longitude);
              }
  ...
  Set<Circle> _circlesFromZones(List<GeofenceZone> zones) {
    return zones
        .where((z) => z.active)
        .map((z) => Circle(
